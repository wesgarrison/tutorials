...
We can create a @before_filter@ which will run _before_ the @new@ and @create@ actions of our @users_controller.rb@.  Open that controller and on the second line you'll see @include AuthenticatedSystem@.  You can remove that since we put it in the @application_controller@ and, in it's place, put all this code:
