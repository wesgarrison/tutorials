...
# If you run into errors, check that you're calling @.new@ with the desired filename, that @initialize@ has the @(filename)@ next to the method name, that @initialize@ is calling @open_file@ with the @filename@ parameter, and that the @open_file@ method has the @(filename)@ next to its method name.
