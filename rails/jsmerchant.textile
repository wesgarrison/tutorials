...
You should then see the @Console@ window open at the bottom of RubyMine with your Mongrel webserver starting up.  Once it's started RubyMine will open your default browser.  If it doesn't open for some reason, try loading the address "http://localhost:3000/":http://localhost:3000.  You should see Rails' "Welcome Aboard" page.  Click the "About your application's environment" link and it'll display the versions of all your installed components.

h4. Generating Your First Scaffold
...
* @config/routes.rb@<br/>When you request a page from the application this is the first place Rails goes.  See the line @map.resources :products@?  That tells Rails that we have something named @products@ and that they should be handled as a @resource@.  Resources are things that follow the RESTful web conventions.  The details are more than you need to know right now, but understand that this line tells Rails that if it gets a request for @/products/@ it should look in the @products_controller.rb@ for a method named @index@
* @app/controllers/products_controller.rb@<br/>After finding the @products@ entry in the Routes file, Rails knows to come to this file and run the @index@ method.  You'll see the @index@ method like the code below.  The second line is the most important -- it reads "Find all the Products and stick them in the variable @@products@".  The lines inside the @respond_to@ block are there to send HTML if the browser is requesting HTML, or send XML if that's what it's expecting.  Ignore them for now.  Once the @index@ method is run Rails looks for the @view@ template.
...
* @app/views/products/index.html.erb@<br/>This is the view template which is responsible for creating the HTML sent to the browser.  The template is written in a format called *erb* which allows us to mix HTML and Ruby.  The first nine lines of this files just looks like plain HTML.  On line 10, though, you see some erb syntax.  Look at the example below.  See the @<%@ and @%>@ in line 11?  Those tags mark the beginning and end of Ruby code inside the erb file.  Basically they mean "evaluate whatever code is between these markers as though it were part of a Ruby program."  Then in line 13 you see a variation where the opening tag has an equals sign like this: @<%=@.  When there is no equals sign, the code between the markers is run *silently*.  When the beginning marker has the _equals sign_, erb will *output* the result of the code into the HTML.  Lastly, the @h@ next to the equals in lines 12/13/14 tell erb that any HTML in this content should be escaped/disabled.
