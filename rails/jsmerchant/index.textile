...
<pre class="brush:ruby">
  def price=(input)
    input.delete!("$") if input
    super
  end
</pre>

The first line looks to see if an @input@ was passed in. If it was, do an in-place delete (@delete!@ as opposed to @delete@), removing any dollar signs. Then, with the value changed, @super@ invokes the original @price=@ method.
...
If you have sample addresses without a "Line 2", you'll see an extra comma in the output. Experiment with using the array method @compact@ to get rid of the blank fields.
...
You'll see that it got a POST request to @"/orders/1"@, which triggers the update action. So we should look at that action in @OrdersController@ to mark the order as submitted.

h4. Recognizing Order Submission

Given our current user interface, we don't need to "edit" orders. Individual order items get edited and updated, but the @Order@ itself doesn't need to be edited/updated. Therefore, we could just re appropriate our @update@ action to have the effect of submitting an order.

But that doesn't feel right. In the future, we might want to allow admins to edit/update orders for customers. Or maybe customers will be able to edit payment methods when a payment fails. This isn't a good long-run solution.

It is a good place to use a custom route and action. Let's build that now.
...
That tells the router that orders will have a custom "member" action -- an action that happens to a single order. The action will use a POST verb and the name "purchase." If you run @rake routes@ in your terminal, you'll see the new listing like this:

<pre class="brush:ruby">
  purchase_order POST   /orders/:id/purchase(.:format)     {:action=>"purchase", :controller=>"orders"}  
</pre>

We chose the POST verb here because the operation we're performing is similar to an edit/update. Since @update@ uses POST, our @purchase@ will too.

h4. Using the Route

In the order's @show.html.erb@ we need to modify the @form_for@ line. Using the default parameters, it is triggering the @update@ action. To send it somewhere else, we use the @:url@ parameter like this:

<pre class="brush:ruby">
  <%= form_for @order, :url => purchase_order_path(@order) do |f| %>
</pre>

Now refresh your order in the browser and click the submit button.

h1. Jeff's Editing Marker

@order.rb@ model file and, under your validations, add this code:
